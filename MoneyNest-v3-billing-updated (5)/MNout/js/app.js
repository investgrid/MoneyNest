...
// ─── LOGO SVGs ────────────────────────────────────────────────
const LOGO_DARK  = `<svg viewBox='0 0 200 44' xmlns='http://www.w3.org/2000/svg' style='width:160px;height:44px'>
  <defs>
...
const LOGO_LIGHT = `<svg viewBox='0 0 200 44' xmlns='http://www.w3.org/2000/svg' style='width:160px;height:44px'>
  <defs>
...
function updateSidebarLogo() {
  ...
  // ── Read plan from MNBilling (authoritative) or fall back to auth ──
  let state = null
  if (window.MNBilling && typeof window.MNBilling.getSubStatus === 'function') {
    state = window.MNBilling.getSubStatus().state
  } else {
    const user = (typeof getUser === 'function') ? getUser() : {}
    const planMap = { pro:'pro_active', local:'local_active', trial:'active_trial', locked_local:'expired_trial' }
    state = planMap[user.plan] || 'active_trial'
  }

  // ── Badge pill styles — superpuesto bottom-right del SVG ──
  const PILL_BASE = `
  ...
  let badgeHtml = `<span id="sidebarProBadge" style="display:none;position:absolute"></span>`

  if (state === 'pro_active' || state === 'pro_trialing') {
    // Morado — ⚡ PRO (como la imagen de referencia)
    badgeHtml = `<span id="sidebarProBadge" style="${PILL_BASE}
      background:linear-gradient(135deg,#A78BFA,#7C3AED); color:#fff;">
      ⚡ PRO
    </span>`
  } else if (state === 'local_active') {
    // Verde — Local
    badgeHtml = `<span id="sidebarProBadge" style="${PILL_BASE}
      background:linear-gradient(135deg,#00D4AA,#059669); color:#fff;">
      💾 Local
    </span>`
  } else if (state === 'active_trial' || state === 'trial_ending') {
    // Ámbar — Trial
    badgeHtml = `<span id="sidebarProBadge" style="${PILL_BASE}
  ...
  // expired_trial / pro_cancelled / syncing → sin badge (logo limpio)

  // Logo siempre centrado; el badge sale del wrapper con position:absolute
  const wrapperHtml = `<span id="logoSvgWrapper" style="position:relative;display:inline-block;line-height:0">${getCurrentLogo()}${badgeHtml}</span>`

  const existing = el.querySelector('#logoSvgWrapper')
  if (existing) {
    existing.outerHTML = wrapperHtml
  } else {
...
function renderInversiones() {
  ...
  const cards = filtradas.map(inv=>{
    ...
    <div class="empty-title">${_invFiltro!=='todas'||_invCat?'Sin resultados':'Sin inversiones'}</div>
    <div class="empty-sub">${_invFiltro!=='todas'||_invCat?'Prueba con otros filtros':'Registra tu primera inversión'}</div>
...
function renderDeudas() {
  ...
  const cards = deudas.map(d=>{
    ...
    <div class="empty-sub">${_deudaSearch||_deudaCatFilter?'Intenta con otros filtros':'Registra tus deudas para hacer seguimiento'}</div>
  ...
  <div class="card" style="margin-bottom:16px">
    ...
    <div style="margin-top:16px;padding-top:14px;border-top:1px solid var(--border)">
      <div style="font-size:.82rem;font-weight:700;color:var(--text);margin-bottom:10px">🧮 Calculadora personalizada</div>
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
        <input type="number" id="deudaCalcInput" placeholder="¿Cuánto puedes pagar al mes? (€)" style="flex:1;min-width:200px;padding:8px 12px;background:var(--bg2);border:1px solid var(--border2);border-radius:var(--radius-sm);color:var(--text);font-size:.85rem">
        <button class="btn btn-primary btn-sm" onclick="calcDeudaPersonalizada()">Calcular</button>
      </div>
      <div id="deudaCalcResult" style="margin-top:10px;font-size:.85rem;color:var(--text2)"></div>
    </div>
  </div>
  <div class="card" style="margin-bottom:16px">
    <div class="card-header">
...
function renderObjetivos() {
  ...
  const cards = filtered.map(obj=>{
    ...
    <div class="empty-icon">🎯</div>
    <div class="empty-title">${_objSearch||_objCatFilter?'Sin resultados':'Aún no tienes objetivos'}</div>
    <div class="empty-sub">${_objSearch||_objCatFilter?'Prueba con otros filtros':'Define tus metas (vacaciones, coche, fondo de emergencia...) y MoneyNest calculará cuándo las alcanzarás'}</div>
...
function renderPresupuestos() {
  ...
  }).join('') || '<div class="empty"><div class="empty-icon">📊</div><div class="empty-title">Aún no tienes presupuestos</div><div class="empty-sub">Establece límites por categoría y controla tu gasto fácilmente — te llevará menos de 30 segundos</div></div>'
...
function renderCuentas() {
  ...
  }).join("") || "<div class=\"empty\"><div class=\"empty-icon\">🏦</div><div class=\"empty-title\">Sin cuentas</div><div class=\"empty-sub\">Añade tus cuentas bancarias</div></div>"
  ...
  <div class="grid-2">
    ...
    <div style="display:flex;flex-direction:column;gap:20px">
      ...
      <div class="card">
        ...
        <div style="display:flex;flex-direction:column;gap:8px">
          <button class="btn btn-secondary btn-sm" onclick="openModal('exportModal')">${t('cfg_exportar_pdf')}</button>
          <button class="btn btn-secondary btn-sm" onclick="exportarJSON()">${t('cfg_backup_json')}</button>
          ...
          <label class="btn btn-secondary btn-sm" style="cursor:pointer;justify-content:flex-start">
            ${t('cfg_importar')}
...
function renderDebtAdvisor() {
  if (!S.deudas.length) return `
    <div class="empty">
      ...
      <div class="empty-sub">Enhorabuena, no tienes deudas registradas.</div>
...
const OB_TOTAL = 3
let obStep  = 1
let obData  = { nombre:'', mode:'personal', lang:'es', theme:'dark', startTutorial:false, loadDemo:false }

// ── Left panel visuals per step ───────────────────────────────
      ...
      <div class="ob-visual-preview">
        ...
        <div class="ob-preview-body" style="flex-direction:column;gap:8px">
          <div class="ob-kpi-row">
            <div class="ob-kpi-mini"><div class="ob-kpi-lbl">Patrimonio</div><div class="ob-kpi-val green">€24.8K</div></div>
            <div class="ob-kpi-mini"><div class="ob-kpi-lbl">Flujo</div><div class="ob-kpi-val blue">+€640</div></div>
            <div class="ob-kpi-mini"><div class="ob-kpi-lbl">Gastos</div><div class="ob-kpi-val red">€1.2K</div></div>
          </div>
          <div class="ob-mini-chart">
            <div class="ob-chart-lbl">Evolución 6 meses</div>
            <div class="ob-chart-bars">
              <div class="ob-bar" style="height:45%;background:rgba(99,102,241,0.5)"></div>
              <div class="ob-bar" style="height:58%;background:rgba(99,102,241,0.5)"></div>
              <div class="ob-bar" style="height:42%;background:rgba(99,102,241,0.5)"></div>
              <div class="ob-bar" style="height:72%;background:rgba(99,102,241,0.5)"></div>
              <div class="ob-bar" style="height:65%;background:rgba(99,102,241,0.5)"></div>
              <div class="ob-bar" style="height:88%;background:#00D4AA"></div>
            </div>
          </div>
          <div class="ob-mini-goal">
            <div class="ob-goal-icon">🎯</div>
            <div class="ob-goal-text"><div class="ob-goal-title">Fondo de emergencia</div><div class="ob-goal-sub">€4.200 de €6.000</div></div>
            <div class="ob-goal-pct">70%</div>
          </div>
        </div>
      </div>
      <div class="ob-feats">
        <div class="ob-feat"><div class="ob-feat-icon">💰</div><div class="ob-feat-text"><strong>${t('ob_feat_ingresos')}</strong>${t('ob_feat_ingresos_sub')}</div></div>
        <div class="ob-feat"><div class="ob-feat-icon">📈</div><div class="ob-feat-text"><strong>${t('ob_feat_inv')}</strong>${t('ob_feat_inv_sub')}</div></div>
        <div class="ob-feat"><div class="ob-feat-icon">🎯</div><div class="ob-feat-text"><strong>${t('ob_feat_obj')}</strong>${t('ob_feat_obj_sub')}</div></div>
      </div>
    </div>`
  if (step === 2) return `
    <div class="ob-left-content">
      ...
      <div style="margin-bottom:24px">
        <div style="font-size:.7rem;font-weight:700;color:rgba(255,255,255,0.3);text-transform:uppercase;letter-spacing:.12em;margin-bottom:12px">${t('ob_left2_title')} ${t('ob_left2_sub')}</div>
        <div style="font-size:1.5rem;font-weight:800;color:#E8EFF7;letter-spacing:-.04em;line-height:1.2;margin-bottom:10px">${t('ob_left2_title')}<br>${t('ob_left2_sub')}</div>
        <div style="font-size:.82rem;color:rgba(255,255,255,0.4);line-height:1.6">${t('ob_left2_desc')}</div>
      </div>
      <div style="display:flex;flex-direction:column;gap:12px;width:100%">
        <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:14px;padding:16px 18px">
          <div style="font-size:.62rem;font-weight:700;color:rgba(0,212,170,0.7);text-transform:uppercase;letter-spacing:.1em;margin-bottom:10px">${t('ob_left2_idioma_lbl')}</div>
          <div style="font-size:1.1rem;font-weight:800;color:#E8EFF7" id="obLeftLang">🇪🇸 Español</div>
        </div>
        <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:14px;padding:16px 18px">
          <div style="font-size:.62rem;font-weight:700;color:rgba(99,102,241,0.8);text-transform:uppercase;letter-spacing:.1em;margin-bottom:10px">${t('ob_left2_tema_lbl')}</div>
          <div style="font-size:1.1rem;font-weight:800;color:#E8EFF7" id="obLeftTheme">🌙 ${t('ob_oscuro')}</div>
        </div>
      </div>
    </div>`
...
function _obRightHTML(step) {
  if (step === 1) return `
    <div class="ob-step-pill"><div class="ob-step-pill-dot"></div>${t('ob_s1_pill')}</div>
    ...
    <div class="ob-name-wrap">
      ...
      <input class="ob-name-input" id="obNombre" type="text" placeholder="${t('ob_s1_placeholder')}" maxlength="40"
        value="${obData.nombre}" oninput="obData.nombre=this.value"
        onkeydown="if(event.key===\'Enter\') obNext()">
    </div>
    <p class="ob-name-hint">${t('ob_s1_hint')}</p>
    <div class="ob-actions-row">
      <button class="ob-next-btn" id="obNext" onclick="obNext()">
  ...
  if (step === 2) {
    const langs = [['es','🇪🇸','Español'],['en','🇺🇸','English'],['it','🇮🇹','Italiano'],['fr','🇫🇷','Français'],['de','🇩🇪','Deutsch'],['pt','🇵🇹','Português']]
    return `
    <div class="ob-step-pill"><div class="ob-step-pill-dot"></div>${t('ob_s2_pill')}</div>
    <div class="ob-headline">${t('ob_s2_h1')}<br>${t('ob_s2_h2')}</div>
    <p class="ob-lead">${t('ob_s2_lead')}</p>
    <div class="ob-section-label">${t('ob_s2_idioma_lbl')}</div>
    <div class="ob-lang-chips" id="obLangChips">
      ${langs.map(([code,flag,name])=>`
        <div class="ob-lang-chip ${obData.lang===code?'selected':''}"
          onclick="obSelectLang(\'${code}\')">
          <span class="ob-lang-chip-flag">${flag}</span>${name}
        </div>`).join('')}
    </div>
    <div class="ob-section-label">${t('ob_s2_tema_lbl')}</div>
    <div class="ob-theme-cards" id="obThemeCards">
      <div class="ob-theme-card ${obData.theme==='dark'?'selected':''}" onclick="obSelectTheme(\'dark\')">
        <div class="ob-theme-mockup dark-mock">
          <div class="ob-mock-topbar dark-bar"><div class="ob-mock-dot"></div><div class="ob-mock-line dark-line"></div></div>
          <div class="ob-mock-body"><div class="ob-mock-card dark-card"></div><div class="ob-mock-card dark-card"></div><div class="ob-mock-card dark-card"></div></div>
        </div>
      ...
      <div class="ob-theme-card ${obData.theme==='light'?'selected':''}" onclick="obSelectTheme(\'light\')">
        <div class="ob-theme-mockup light-mock">
          <div class="ob-mock-topbar light-bar"><div class="ob-mock-dot"></div><div class="ob-mock-line light-line"></div></div>
          <div class="ob-mock-body"><div class="ob-mock-card light-card"></div><div class="ob-mock-card light-card"></div><div class="ob-mock-card light-card"></div></div>
        </div>
    ...
    <div class="ob-actions-row">
      ...
      <button class="ob-next-btn" id="obNext" onclick="obNext()">
        ${t('ob_s2_siguiente')} <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style="display:inline;vertical-align:middle"><path d="M3 8h10M9 4l4 4-4 4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
      </button>
    </div>`
  }
  ...
  const nombre = obData.nombre || 'Usuario'
  return `
    <div class="ob-greeting-badge"><span class="ob-greeting-badge-text">${t('ob_s3_greeting')}, ${nombre}! 🎉</span></div>
    ...
    <div class="ob-actions-row">
      <button class="ob-back-btn" onclick="obPrev()">${t('ob_atras')}</button>
      <button class="ob-next-btn" id="obNext" onclick="obNext()">${t('ob_s3_cta')}</button>
    </div>`
}
...
function obSelectOpt(mode) {
  ...
  document.querySelectorAll('#obOptions .ob-opt').forEach((el, i) => {
    const isSelected = (mode==='tutorial'&&i===0)||(mode==='demo'&&i===1)||(mode==='direct'&&i===2)
...
function obRender(direction) {
  ...
  if (leftPanel) leftPanel.innerHTML = _obLeftHTML(obStep)
  ...
  setTimeout(() => {
    ...
    if (obStep === 1) setTimeout(() => document.getElementById('obNombre')?.focus(), 80)
    _obUpdateLeftLive()
  }, 160)
}

function _obUpdateLeftLive() {
  const langEl = document.getElementById('obLeftLang')
  const themeEl = document.getElementById('obLeftTheme')
  const langs = {es:'🇪🇸 Español',en:'🇺🇸 English',it:'🇮🇹 Italiano',fr:'🇫🇷 Français',de:'🇩🇪 Deutsch',pt:'🇵🇹 Português'}
  if (langEl) langEl.textContent = langs[obData.lang] || '🇪🇸 Español'
  if (themeEl) themeEl.textContent = obData.theme === 'dark' ? `🌙 ${t('ob_oscuro')}` : `☀️ ${t('ob_claro')}`
}

function obSelectLang(l) {
  obData.lang = l
  _currentLang = l
  // Re-render both panels instantly so all text reflects the new language
  const leftPanel = document.getElementById('obLeftPanel')
  const contentArea = document.getElementById('obContentArea')
  if (leftPanel) leftPanel.innerHTML = _obLeftHTML(obStep)
  if (contentArea) {
...
function obSelectTheme(t) {
  ...
  document.querySelectorAll('#obThemeCards .ob-theme-card').forEach((el, i) => {
    const sel = (t === 'dark' && i === 0) || (t === 'light' && i === 1)
...
function finishOnboarding() {
  ...
  if (obData.theme) S.theme = obData.theme
...
function checkOnboarding() {
  if (!_obFlagSeen()) {
    obStep = 1
    obData = { nombre:'', mode:'personal', lang:'es', theme:'dark', startTutorial:false, loadDemo:false }
...
function renderPatrimonio() {
  ...
  const assetsListHtml = activeAssets.length
    ...
    : `<div style="padding:28px;text-align:center;color:var(--text2)">
        ...
        <div style="font-size:.85rem;font-weight:600">Sin activos físicos</div>
        <div style="font-size:.78rem;margin-top:4px">Añade propiedades, vehículos u otros bienes</div>
      </div>`
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px" class="pat-split-grid">
    ...
    <div style="display:flex;flex-direction:column;gap:16px">
      ...
      <div class="card">
        ...
        ${cuentasHtml||'<div class="empty"><div class="empty-icon">🏦</div><div class="empty-title">Sin cuentas</div></div>'}
      </div>
      ...
      <div class="card">
        ...
        ${S.patrimonio_hist.length>=2
          ...
          :`<div class="empty" style="padding:24px"><div class="empty-icon">📈</div><div class="empty-title">Pocos datos</div><div class="empty-sub">Vuelve el mes que viene para ver la evolución</div></div>`}
      </div>
    </div>
...
function _showGuestGateModal() {
  ...
  modal.innerHTML = `
    <div style="background:var(--card);border:1px solid var(--border2);border-radius:24px;width:min(420px,calc(100vw-32px));padding:36px 32px;text-align:center;box-shadow:0 40px 100px rgba(0,0,0,.6);animation:smCardIn .4s cubic-bezier(0.22,1,0.36,1) forwards">
      ...
      <div style="font-size:1.25rem;font-weight:800;color:var(--text);letter-spacing:-.04em;margin-bottom:8px">${t('guest_gate_title')}</div>
      <div style="font-size:.85rem;color:var(--text2);line-height:1.6;margin-bottom:28px">${t('guest_gate_desc')}</div>
      <div style="display:flex;flex-direction:column;gap:10px">
        <button onclick="_authStartTrial()" style="padding:14px;border-radius:12px;border:none;background:linear-gradient(135deg,#6366F1,#4F46E5);color:#fff;font-size:.9rem;font-weight:800;cursor:pointer;font-family:inherit;box-shadow:0 4px 16px rgba(99,102,241,.35)">🕐 Iniciar prueba gratuita (${TRIAL_DAYS} días)</button>
        <button onclick="_authActivatePro()" style="padding:13px;border-radius:12px;border:none;background:linear-gradient(135deg,#00D4AA,#00A882);color:#0A0E17;font-size:.875rem;font-weight:800;cursor:pointer;font-family:inherit">⚡ Activar Pro directamente</button>
        <button onclick="document.getElementById('guestGateModal').style.display='none'" style="padding:11px;border-radius:12px;border:1.5px solid var(--border2);background:transparent;color:var(--text3);font-size:.8rem;font-weight:600;cursor:pointer;font-family:inherit">Continuar explorando</button>
      </div>
    </div>`
